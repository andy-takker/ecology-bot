...
def upgrade() -> None:
    ...
    op.drop_index(
        "ix_district_organization_district_id", table_name="district_organization"
    )
    op.drop_index("ix_district_organization_id", table_name="district_organization")
    op.drop_index(
        "ix_district_organization_organization_id", table_name="district_organization"
    )
    op.drop_table("district_organization")
    op.alter_column(
        "employee", "login", existing_type=sa.VARCHAR(length=30), nullable=False
    )
    op.create_unique_constraint(None, "employee", ["login"])
    op.add_column(
        "organization", sa.Column("district_id", sa.BigInteger(), nullable=False)
    )
    op.create_index(
        op.f("ix_organization_district_id"),
        "organization",
        ["district_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.alter_column(
        "employee", "login", existing_type=sa.VARCHAR(length=30), nullable=True
    )
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["district_id"],
            ["district.id"],
            name="district_organization_district_id_fkey",
        ),
        sa.ForeignKeyConstraint(
            ["organization_id"],
            ["organization.id"],
            name="district_organization_organization_id_fkey",
        ),
        sa.PrimaryKeyConstraint("id", name="district_organization_pkey"),
    )
    op.create_index(
        "ix_district_organization_organization_id",
        "district_organization",
        ["organization_id"],
        unique=False,
    )
    op.create_index(
        "ix_district_organization_id", "district_organization", ["id"], unique=False
    )
    op.create_index(
        "ix_district_organization_district_id",
        "district_organization",
        ["district_id"],
        unique=False,
    )

...
from aiogram_dialog.widgets.kbd import Column, Select, Cancel, Start, SwitchTo
from aiogram_dialog.widgets.text import Const, Format

from ecology_bot.bot.dialogs.states import ProfileManagementSG, ProfileDeleteSG, RegisterVolunteerSG
from ecology_bot.bot.services.repo import Repo


async def show_organization_by_activity(c: CallbackQuery, widget: Any, manager: DialogManager, item_id: str):
    ...
    if organizations:
        ...
        msg.append('В Вашем районе не зарегистрированы экологические организации с таким направлением.')
        msg.append('Возможно, они появятся позже, а пока можете выбрать другой район или направление')
...
async def profile_window_data(dialog_manager: DialogManager, **kwargs):
    ...
    profile = await repo.user_dao.get_profile(telegram_id=dialog_manager.event.from_user.id)
    return {

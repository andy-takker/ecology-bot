...
from aiogram_dialog.widgets.kbd import Cancel, Back, ScrollingGroup, Select, Multiselect, Button
from aiogram_dialog.widgets.text import Const, Format

from ecology_bot.bot.dialogs.states import RegisterVolunteerSG
...
async def complete_volunteer_registration(c: CallbackQuery, widget: Any, manager: DialogManager):
    ...
    volunteer_types = await repo.volunteer_type_dao.get_volunteer_types(volunteer_type_ids)
    ...
    await c.message.answer('Теперь тебе будут приходить уведомления об '
...
def get_volunteer_type_window() -> Window:
    ...
    return Window(
        ...
        volunteer_types,
        Button(text=Const('Завершить регистрацию', ), id='finish_profile', on_click=complete_volunteer_registration),
        Back(text=Const('Назад')),

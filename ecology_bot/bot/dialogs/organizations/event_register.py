...
from aiogram_dialog import Dialog, DialogManager, Data, ShowMode
from aiogram_dialog.widgets.kbd import Cancel, Back
from loguru import logger

from ecology_bot.bot.dialogs.states import CreateEventSG
...
from ecology_bot.bot.windows.event_type_window import EventTypeWindow
from ecology_bot.bot.windows.input_text_window import InputTextWindow
from ecology_bot.bot.windows.volunteer_type_window import VolunteerTypeWindow
from ecology_bot.database import Organization, EventType
...
async def get_name_prev_state(c: CallbackQuery, widget: Any, manager: DialogManager):
    if EventType(manager.current_context().dialog_data['event_type']) == EventType.DEFAULT:
...
async def on_start(data: Data, manager: DialogManager):
    ...
    region_id = (await repo.district_dao.get_district(district_id=organization.district_id)).region_id
...
async def on_finish(c: CallbackQuery, widget: Any, manager: DialogManager):
    ...
    if EventType(data['event_type']) == EventType.RECRUITMENT:
        volunteer_types = await repo.volunteer_type_dao.get_volunteer_types(ids=data['volunteer_type'])
...
def get_dialog():
    ...
    activity_window = ActivityWindow(state=CreateEventSG.activity, prev=Back, all=False,prev_state=CreateEventSG.event_type)
